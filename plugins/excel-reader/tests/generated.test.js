...
const excelDirecotry = path.join(__dirname, '/assets/fixtures/excel')

if (!fs.existsSync(excelDirecotry)) fs.mkdirSync(excelDirecotry)

const mergedWB = XLSX.utils.book_new()
globSync(__dirname + '/assets/fixtures/**/**.csv').forEach((file) => {
	const fileName = path.parse(file).name
	let destination = path.join(excelDirecotry, `${fileName}.xlsx`)
...
XLSX.writeFile(mergedWB, path.join(excelDirecotry, `merged.xlsx`))
...
describe('generated excel read', () => {
	test.each(generatedCases())(
		...
		({ fileName, expected, detected, sheet = 0 }) => {
			...
			const response = excelReader.readContent(`${excelDirecotry}/${fileName}`, {
				fileType: 'file',
				sheet: sheet
			})

...
const percentOfTypes = (rowValues: string[]): number => {
  const filteredItems = rowValues.filter(
...
const convertValuesToTypes = (rowValues: string[][], missingHeaderElements: number[]): string[][] => {
  const filteredValues = missingHeaderElements.length > 0 ? rowValues.map(row => row.filter((_, index) => !missingHeaderElements.includes(index))): rowValues;
...
export const headerDetect = (data: string, delimiter: string = ","): number => {
  ...
  const headerTypesProbability = percentOfTypes(clearHeaderTypes.length > 0 ? clearHeaderTypes : headerTypes);
